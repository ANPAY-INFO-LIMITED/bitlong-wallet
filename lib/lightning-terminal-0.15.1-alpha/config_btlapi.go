...
var (
	lndDefaultConfig = func() lnd.Config {
		lndConfig := lnd.DefaultConfig()
		// 默认配置设置为200,但配置文件中设置为125,最终应该是125
		lndConfig.NeutrinoMode.MaxPeers = 200
		lndConfig.NeutrinoMode.PersistFilters = true
		return lndConfig
	}()
...
func loadAndValidateConfig(interceptor signal.Interceptor) (*Config, error) {
	...
	preCfg.LitDir = base.Configure("lit")
	preCfg.Lnd.LndDir = base.Configure("lnd")
	preCfg.TaprootAssets.TapdDir = base.Configure("tapd")
	preCfg.Loop.LoopDir = base.Configure("loop")
	preCfg.Pool.BaseDir = base.Configure("pool")
	preCfg.Faraday.FaradayDir = base.Configure("faraday")
