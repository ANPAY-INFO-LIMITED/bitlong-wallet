...
func (h *OrderHandler) handleIncomingHtlc(ctx context.Context,
	...
	if _, ok := policy.(*AssetPurchasePolicy); ok {
		SaveIncomingHtlcIds(htlc.IncomingCircuitKey.HtlcID)
		fmt.Println("htlc is AssetPurchasePolicy", htlc.IncomingCircuitKey.HtlcID)
	}
	if _, ok := policy.(*AssetSalePolicy); ok {
		fmt.Println("htlc is AssetSalePolicy", htlc.IncomingCircuitKey.HtlcID)
	}
	if _, ok := policy.(*AssetForwardPolicy); ok {
		SaveIncomingHtlcIds(htlc.IncomingCircuitKey.HtlcID)
		fmt.Println("htlc is AssetForwardPolicy", htlc.IncomingCircuitKey.HtlcID)
	}
