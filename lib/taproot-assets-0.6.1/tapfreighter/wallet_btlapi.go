...
type Wallet interface {
	...
	// FundPacketListEligibleCoins btlapi 调用内层实现
	FundPacketListEligibleCoins(ctx context.Context, fundDesc *tapsend.FundingDescriptor) ([]*AnchoredCommitment, error)

	// FundPacketWithFilter btlapi
	FundPacketWithFilter(ctx context.Context, fundDesc *tapsend.FundingDescriptor, vPkt *tappsbt.VPacket, coinsFilter *btlrpc.CoinsFilter) (*FundedVPacket, error)

	// FundAddressSendWithFilter btlapi
	FundAddressSendWithFilter(ctx context.Context, coinSelectType fn.Option[asset.ScriptKeyType], coinsFilter *btlrpc.CoinsFilter, receiverAddrs ...*address.Tap) (*FundedVPacket, error)
}
...
// FundAddressSendWithFilter btlapi
func (f *AssetWallet) FundAddressSendWithFilter(ctx context.Context, coinSelectType fn.Option[asset.ScriptKeyType], coinsFilter *btlrpc.CoinsFilter, receiverAddrs ...*address.Tap) (*FundedVPacket, error) {
	...
	fundDesc.ScriptKeyType = coinSelectType
	// btlapi
	fundedVPkt, err := f.FundPacketWithFilter(ctx, fundDesc, vPkt, coinsFilter)
	if err != nil {
		return nil, err
	}
...
// FundPacketListEligibleCoins btlapi
func (f *AssetWallet) FundPacketListEligibleCoins(ctx context.Context, fundDesc *tapsend.FundingDescriptor) ([]*AnchoredCommitment, error) {
	...
	selectedCommitments, err := f.cfg.CoinSelector.SelectCoinsListEligibleCoins(
		ctx, constraints,
	)
	if err != nil {
		return nil, err
...
// FundPacketWithFilter btlapi
func (f *AssetWallet) FundPacketWithFilter(ctx context.Context, fundDesc *tapsend.FundingDescriptor, vPkt *tappsbt.VPacket, coinsFilter *btlrpc.CoinsFilter) (*FundedVPacket, error) {
	...
	// btlapi
	selectedCommitments, err := f.cfg.CoinSelector.SelectCoinsWithFilter(
		ctx, constraints, PreferMaxAmount, *anchorVersion, coinsFilter,
	)
	if err != nil {
		return nil, err
	}
