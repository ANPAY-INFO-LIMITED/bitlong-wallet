...
// SelectCoinsListEligibleCoins btlapi
func (s *CoinSelect) SelectCoinsListEligibleCoins(ctx context.Context, constraints CommitmentConstraints) ([]*AnchoredCommitment, error) {
...
// SelectCoinsWithFilter btlapi
func (s *CoinSelect) SelectCoinsWithFilter(ctx context.Context, constraints CommitmentConstraints, strategy MultiCommitmentSelectStrategy, maxVersion commitment.TapCommitmentVersion, coinsFilter *btlrpc.CoinsFilter) ([]*AnchoredCommitment, error) {
	...
	// @Note: btlapi: 通过给定的过滤器过滤硬币
	if coinsFilter == nil || len(coinsFilter.Coins) == 0 {
		// 允许不指定过滤器，则获取所有可用硬币
	} else {
		var FilteredEligibleCommitments []*AnchoredCommitment
		filterCoinsExistMap := make(map[string]bool)
		filterCoinsMap := make(map[string]*btlrpc.Coin)
		for _, coin := range coinsFilter.Coins {
			filterCoinsMap[coin.AnchorPoint] = coin
			filterCoinsExistMap[coin.AnchorPoint] = true
		}
		for _, ec := range eligibleCommitments {
			ecAnchorPoint := ec.AnchorPoint.String()
			if _, ok := filterCoinsExistMap[ecAnchorPoint]; ok {
				ecInternalKey := ec.InternalKey.PubKey.SerializeCompressed()
				fcInternalKey := filterCoinsMap[ecAnchorPoint].InternalKey
				// 当过滤条件提供了内部密钥时，则判断是否相等，否则只需要满足锚定点相同即可
				if (len(fcInternalKey) == len(ecInternalKey) && bytes.Equal(ecInternalKey, fcInternalKey)) || len(fcInternalKey) == 0 {
					FilteredEligibleCommitments = append(FilteredEligibleCommitments, ec)
				}
				continue
			}
			continue
		}
		if len(FilteredEligibleCommitments) == 0 {
			return nil, errors.New("no coins found after filtering")
		}
		eligibleCommitments = FilteredEligibleCommitments
	}
