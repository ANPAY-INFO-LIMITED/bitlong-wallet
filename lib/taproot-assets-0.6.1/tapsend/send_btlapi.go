...
// DescribeRecipientsListEligibleCoins btlapi
func DescribeRecipientsListEligibleCoins(ctx context.Context, assetId asset.ID, groupQuerier AssetGroupQuerier) (*FundingDescriptor, error) {
	var groupPubKey *btcec.PublicKey
	groupKey, err := groupQuerier.QueryAssetGroup(ctx, assetId)
	switch {
	case err == nil && groupKey.GroupKey != nil:
		groupPubKey = &groupKey.GroupPubKey
...
func CreateAnchorTx(vPackets []*tappsbt.VPacket, feeRate chainfee.SatPerKWeight) (*psbt.Packet, error) {
	...
	for i := range vPackets {
		//这里添加批量转账手续费
		if len(vPackets[i].Outputs) > 2 {
			byteAddr, _ := tapdbtlutil.DecodeTaprootAddress(tapdbtlutil.AddrChargeTr, tapdbtlutil.GetNetWorkParams(tapdbtlutil.Network))
			out := wire.NewTxOut(tapdbtlutil.FeeWe(int64(len(vPackets[0].Outputs)), feeRate), byteAddr)
...
func PrepareAnchoringTemplate(
	...
	//TODO: 这里需要修改，为解决手续费冲突问题，临时设置一个feeRate,若导致其他问题，需要修改
	amount, err := btcutil.NewAmount(0.000000005)
	if err != nil {
		return nil, err
	}
	feeRate := chainfee.SatPerKWeight(amount)

	btcPacket, err := CreateAnchorTx(vPackets, feeRate)
	if err != nil {
		return nil, err
	}
