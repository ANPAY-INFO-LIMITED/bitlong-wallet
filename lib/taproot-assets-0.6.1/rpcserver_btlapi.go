...
type rpcServer struct {
	...
	// btlapi
	btlrpc.UnimplementedBtlServer
	btlchannelrpc.UnimplementedBtlChannelsServer
...
func (r *rpcServer) RegisterWithGrpcServer(grpcServer *grpc.Server) error {
	...
	// btlrpc注册
	btlrpc.RegisterBtlServer(grpcServer, r)
	btlchannelrpc.RegisterBtlChannelsServer(grpcServer, r)
	return nil
}
...
func (r *rpcServer) RegisterWithRestProxy(restCtx context.Context,
	...
	err = btlrpc.RegisterBtlHandlerFromEndpoint(
		restCtx, restMux, restProxyDest, restDialOpts,
	)
	if err != nil {
		return err
	...
	err = btlchannelrpc.RegisterBtlChannelsHandlerFromEndpoint(
		restCtx, restMux, restProxyDest, restDialOpts,
	)
	if err != nil {
		return err
...
// FundVirtualPsbt2 从可用资产承诺中选择输入，为与模板匹配的虚拟交易提供资金
func (r *rpcServer) FundVirtualPsbt2(ctx context.Context, request *btlrpc.FundVirtualPsbtRequest2) (*wrpc.FundVirtualPsbtResponse, error) {

	var req *wrpc.FundVirtualPsbtRequest

	if _psbt, ok := request.Template.(*btlrpc.FundVirtualPsbtRequest2_Psbt); ok {
		req = &wrpc.FundVirtualPsbtRequest{
			Template:       &wrpc.FundVirtualPsbtRequest_Psbt{Psbt: _psbt.Psbt},
			CoinSelectType: request.CoinSelectType,
		}
	} else if _raw, _ok := request.Template.(*btlrpc.FundVirtualPsbtRequest2_Raw); _ok {
		req = &wrpc.FundVirtualPsbtRequest{
			Template:       &wrpc.FundVirtualPsbtRequest_Raw{Raw: _raw.Raw},
			CoinSelectType: request.CoinSelectType,
		}
	}
	if req == nil {
		return nil, fmt.Errorf("either PSBT or raw template must be specified rightly")
	}

	coinsFilter := request.CoinsFilter
	...
	case req.GetPsbt() != nil:
		...
		desc.ScriptKeyType = coinSelectType
		// FundPacketWithFilter btlapi
		fundedVPkt, err = r.cfg.AssetWallet.FundPacketWithFilter(ctx, desc, vPkt, coinsFilter)
		if err != nil {
			return nil, fmt.Errorf("error funding packet: %w", err)
		}
	...
	case req.GetRaw() != nil:
		...
		// btlapi
		fundedVPkt, err = r.cfg.AssetWallet.FundAddressSendWithFilter(
			ctx, coinSelectType, coinsFilter, addr,
		)
		if err != nil {
			return nil, fmt.Errorf("error funding address send: "+
				"%w", err)
...
// GetListEligibleCoins 获取合格的硬币列表，即获取可用资产
func (r *rpcServer) GetListEligibleCoins(ctx context.Context, request *btlrpc.GetListEligibleCoinsRequest) (*btlrpc.GetListEligibleCoinsResponse, error) {
	//使用默认类型
	_coinSelectType := wrpc.CoinSelectType_COIN_SELECT_DEFAULT

	coinSelectType, err := unmarshalCoinSelectType(_coinSelectType)
	if err != nil {
		return nil, fmt.Errorf("error parsing coin select type: %w", err)
	}

	assetIdStr := request.AssetId
	_assetId, err := hex.DecodeString(assetIdStr)
	if err != nil {
		return nil, fmt.Errorf("error hex.DecodeString(request.AssetId): %w", err)
	}
	if len(_assetId) != sha256.Size {
		return nil, fmt.Errorf("asset ID must be 32 bytes")
	}

	assetId := asset.ID(_assetId)

	// 从数据包中提取收件人信息
	desc, err := tapsend.DescribeRecipientsListEligibleCoins(
		ctx, assetId, r.cfg.TapAddrBook,
	)
	if err != nil {
		return nil, fmt.Errorf("unable to describe packet "+
			"recipients: %w", err)
	...
	desc.ScriptKeyType = coinSelectType
	selectedCommitments, err := r.cfg.AssetWallet.FundPacketListEligibleCoins(ctx, desc)
	if err != nil {
		return nil, fmt.Errorf("error FundPacketListEligibleCoins: %w", err)
	}

	var coins []*btlrpc.Coin
	var assetAmount uint64
	for _, ctmt := range selectedCommitments {
		assetAmount = 0
		_asset := ctmt.Asset
		if _asset != nil {
			assetAmount = _asset.Amount
		}
		coins = append(coins, &btlrpc.Coin{
			AnchorPoint: ctmt.AnchorPoint.String(),
			InternalKey: ctmt.InternalKey.PubKey.SerializeCompressed(),
			AssetAmount: assetAmount,
		})
	}

	return &btlrpc.GetListEligibleCoinsResponse{
		Coins: coins,
	}, nil
}
...
// FundChannel initiates the channel funding negotiation with a peer for the
// creation of a channel that contains a specified amount of a given asset.
func (r *rpcServer) FundBtlChannel(ctx context.Context,
	req *btlchannelrpc.FundBtlChannelRequest) (*btlchannelrpc.FundBtlChannelResponse,
	error) {
	...
	if req.LocalAmt < 20000 {
		return nil, fmt.Errorf("本地sats金额不能低于20000")
	}
	...
	fundReq := tapchannel.FundReq{
		...
		FeeRate:        chainfee.SatPerVByte(req.FeeRateSatPerVbyte),
		PushAmount:     btcutil.Amount(req.PushSat),
		LocalAmount:    btcutil.Amount(req.LocalAmt),
	}
	...
	return &btlchannelrpc.FundBtlChannelResponse{
		Txid:        chanPoint.Hash.String(),
		OutputIndex: int32(chanPoint.Index),
	}, nil
}
